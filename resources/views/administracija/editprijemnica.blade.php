...
@section('tab-content')
    ...
    <form method="POST" action="{{route('updatePrijemnica',$prijemnica->id)}}">
        @csrf
        @method('PATCH')
        <div class="flex-lg-row">
            <span class="text-light font-weight-bold mx-4">Broj prijemnice: {{$prijemnica->BrDok}}</span>
            <span class="text-light font-weight-bold mx-4">Datum prijemnice: {{date_format($prijemnica->created_at,"d/m/Y")}}</span>
        ...
        <div id="bar" class="col-2 flex-row">
            ...
            <select id="komitent" @if($prijemnica->IndikatorKnjizenja) disabled @endif  class="form-control  dropstatic" name="komitent" required>
                <option  disabled value="">Izaberi Dobavljaca</option>
                @foreach($komitenti as $komitent)
                    <option value="{{$komitent->Sifra}}" @if($prijemnica->SifKom==$komitent->Sifra) selected @endif>{{$komitent->Naziv}}</option>
                @endforeach
            </select>
            <label for="brdok" class="font-weight-bold text-light">Broj dokumenta</label>
            <input name="brdok" id="brdok" @if($prijemnica->IndikatorKnjizenja) disabled @endif value="{{$prijemnica->BrFiskal}}" required>
        </div>
        <div>
            <div class="form-group">
                <table class="table table-borderless" id="tabelakomponenti">
                    ...
                    <tr class="font-weight-bold text-light">
                        ...
                        <th>NC bez PDV</th>
                        <th>NC sa PDV</th>
                        @if(!$prijemnica->IndikatorKnjizenja)<th><button type="button" class="btn btn-success btn-sm" id="btnadd">+</button></th>@endif
                    </tr>
                    </thead>
                    <tbody>
                ...
                <div id="napomenadiv" class="col-2">
                    ...
                    <textarea id="napomena" @if($prijemnica->IndikatorKnjizenja) disabled @endif name="napomena">{{$prijemnica->Napomena}}</textarea>
                </div>
                <div class="float-right">
                    <a href="{{route('indexPrijemnica')}}" class="btn btn-warning">Nazad</a>
                    @if(!$prijemnica->IndikatorKnjizenja)
                    <button type="submit" name="sub" class="btn btn-success ">Sacuvaj izmenu</button>
                    @endif
                </div>
            </div>
        </div>

...
@section('orgjed')
    <div class="card-body">
        <form method="POST" action="{{$edit ? route('updateOrganizacionajedinica',$organizacionajedinica->SifOj) : route('storeOrganizacionajedinica')}}">
            @csrf
            @if($edit)
                @method('PATCH')
            @endif
            <div class="form-group row">
                <label for="vrsta" class="col-md-4 col-form-label text-md-right">Vrsta</label>
            ...
            <div class="form-group row mb-0">
                <div class="col-md-8 offset-md-4">
                    <button type="submit" class="btn btn-primary">
                        @if($edit) Izmeni @else Dodaj @endif organizacionu jedinicu
                    </button>
                </div>
            </div>
        </form>

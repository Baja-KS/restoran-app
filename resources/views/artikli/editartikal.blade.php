            ...
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <div>
                        ...
                        <input id="prodavnica" type="text" class="form-control @error('prodavnica') is-invalid @enderror" name="prodavnica" value="{{ $artikal->magacin->Prodavnica }}" required autocomplete="naziv" autofocus>
                    </div>
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <div>
                        ...
                        <input id="kolicinaulaza" type="number" min="0" step="0.01" class="form-control @error('kolicinaulaza') is-invalid @enderror" name="kolicinaulaza" value="{{ $artikal->magacin->KolicinaUlaza }}">
                    </div>
                    <div>
                        <label for="kolicinaizlaza" class="col-form-label">Kolicina Izlaza</label>
                        <input id="kolicinaizlaza" type="number" min="0" step="0.01" class="form-control @error('kolicinaizlaza') is-invalid @enderror" name="kolicinaizlaza" value="{{ $artikal->magacin->KolicinaIzlaza }}">
                    </div>
                    <div>
                        <label for="zadnjaprodajnacena" class="col-form-label">Zadnja Prodajna Cena</label>
                        <input id="zadnjaprodajnacena" min="0"  type="number" step="0.1" class="form-control @error('zadnjaprodajnacena') is-invalid @enderror" name="zadnjaprodajnacena" value="{{ $artikal->magacin->ZadnjaProdajnaCena }}">
                    </div>
                    <div>
                        <label for="zadnjanabavnacena" class="col-form-label">Zadnja Nabavna Cena</label>
                        <input id="zadnjanabavnacena" type="number" min="0" step="0.1" class="form-control @error('zadnjanabavnacena') is-invalid @enderror" name="zadnjanabavnacena" value="{{ $artikal->magacin->ZadnjaNabavnaCena }}">
                    </div>
                </div>
            </div>
            <div class="col-md-2">
                    ...
                    <table class="table table-borderless" id="tabelakomponenti">
                        ...
                        <tbody>
                            @foreach($artikal->komponente as $komponenta)
                                <tr>
                                    <td><select class="form-control dropdown" name="komponenta[]"><option value="">Izaberi komponentu</option> @foreach($artikli->where('Normativ',false) as $kartikal)<option value="{{$kartikal->PLUKod}}" @if($komponenta->PLUKod==$kartikal->PLUKod) selected @endif>{{$kartikal->Naziv}}</option> @endforeach </select></td>
                                    <td><input type="number" class="form-control" min="0" step="0.01" name="kolicina[]" value="{{\App\Artikal::kolicinaUMesavini($artikal,$komponenta)}}"></td>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click','#btnadd',function () {
                var html='';
                html+='<tr>';
                html+='<td><select class="form-control dropdown" name="komponenta[]"><option value="">Izaberi komponentu</option> @foreach($artikli->where('Normativ',false) as $kartikal)<option value="{{$kartikal->PLUKod}}">{{$kartikal->Naziv}}</option> @endforeach </select></td>';
                html+='<td><input type="number" min="0" step="0.01" class="form-control" name="kolicina[]"></td>';
